...
ndr-batch:
	bash ./execute-ndr-scenario-batch.sh >> $@-$(DATE)-$(GIT_REV).log
...
all:
	bash ./execute-ndr-scenario-batch.sh --with-noxn 10km >> $@-$(DATE)-$(GIT_REV).log

all-1km:
	bash ./execute-ndr-scenario-batch.sh --with-noxn 1km >> $@-$(DATE)-$(GIT_REV).log
